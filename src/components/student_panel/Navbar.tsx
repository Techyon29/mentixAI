  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between py-4 mb-4 bg-[#F0F5FA]/80 backdrop-blur-md rounded-b-2xl">
      ...
      <div className="flex items-center gap-6 pr-2">
        ...
        {hasLiveTest && (
          <Link href="/student-dashboard/assessments" className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-red-50 border border-red-100 rounded-full group hover:bg-red-100 transition-colors shadow-sm cursor-pointer">
             <div className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span>
             </div>
             <span className="text-[11px] font-black text-red-600 uppercase tracking-wider group-hover:tracking-widest transition-all">Live Test</span>
          </Link>
        )}
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <img src="https://i.pravatar.cc/150?u=r1" alt="Rohan Verma" className="w-10 h-10 rounded-full border-2 border-white shadow-sm group-hover:shadow-md transition-all object-cover" />
            <div className="flex flex-col">
              <span className="text-[#0D3694] font-bold text-[14.5px]">Rohan Verma</span>
              <span className="text-[#5B779E] text-[12.5px] font-medium">Class 10</span>
            </div>
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-3 w-64 bg-white/90 backdrop-blur-2xl border border-white rounded-[24px] shadow-[0_10px_40px_rgba(0,0,0,0.08)] p-2 animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden">
              <div className="px-4 py-4 mb-2 border-b border-slate-100">
                <p className="text-[12px] font-black text-slate-400 uppercase tracking-widest mb-1">Student Account</p>
                <p className="text-[14px] font-bold text-[#0D3694] truncate">rohan.verma@school.edu</p>
              </div>
