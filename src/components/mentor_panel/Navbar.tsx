      ...
      <div className="flex items-center gap-2 md:gap-4 flex-1">
        ...
        <div className="relative w-full max-w-[480px] hidden sm:block">
          ...
          <input 
            ...
            className="w-full pl-9 md:pl-11 pr-4 py-1.5 md:py-2 bg-white/40 backdrop-blur-md border border-white/60 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.02),_inset_0_2px_4px_rgba(255,255,255,0.8)] focus:outline-none focus:ring-2 focus:ring-blue-400/50 text-[#0D3694] placeholder:text-slate-500 font-medium transition-all text-[11px] md:text-[13px]"
          />
        </div>
        <button className="p-1.5 bg-white/40 border border-white/60 rounded-xl sm:hidden text-[#0D3694] shrink-0">
          <Search className="w-4 h-4" />
        </button>
      </div>
      <div className="flex items-center gap-1.5 md:gap-6 pr-1 md:pr-2 shrink-0">
        ...
        <div className="relative" ref={notificationsRef}>
          <button 
            ...
            className={`relative p-1.5 md:p-2 transition-all rounded-full border border-transparent ${isNotificationsOpen ? 'bg-white shadow-md border-slate-100 text-blue-600' : 'text-[#0D3694] hover:bg-white/40'}`}
          >
            <Bell className="w-4 h-4 md:w-5 md:h-5" />
            {unreadCount > 0 && (
              <span className="absolute top-1 right-1 md:top-1.5 md:right-1.5 w-2 h-2 md:w-2.5 md:h-2.5 bg-red-500 border-2 border-white rounded-full animate-pulse shadow-sm"></span>
            )}
          </button>
          ...
          {isNotificationsOpen && (
            <div className="absolute right-0 mt-3 w-72 md:w-80 bg-white/90 backdrop-blur-2xl border border-white rounded-[24px] md:rounded-[32px] shadow-[0_10px_40px_rgba(0,0,0,0.08)] p-2 md:p-3 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <div className="max-h-[300px] md:max-h-[400px] overflow-y-auto custom-scrollbar py-1 md:py-2">
                {notifications.map((n) => (
        ...
        <div className="relative" ref={dropdownRef}>
          <div 
            ...
            className={`flex items-center gap-1.5 md:gap-3 cursor-pointer group px-2 md:px-3 py-1 md:py-1.5 rounded-xl md:rounded-2xl transition-all border border-transparent ${isDropdownOpen ? 'bg-white shadow-md border-slate-100' : 'hover:bg-white/40 hover:border-white/60'}`}
          >
            <img src="https://i.pravatar.cc/150?u=a042581f4e29026704d" alt="Arjun" className="w-7 h-7 md:w-10 md:h-10 rounded-full border-2 border-white shadow-sm group-hover:shadow-md transition-all object-cover shrink-0" />
            <div className="hidden sm:flex flex-col">
              <span className="text-[#0D3694] font-bold text-[12px] md:text-[14.5px]">Arjun Mehta</span>
              <span className="text-[#5B779E] text-[10px] md:text-[12.5px] font-medium">Mentor</span>
            </div>
            <ChevronDown className={`w-3 h-3 md:w-4 md:h-4 text-[#5B779E] ml-0.5 md:ml-1 transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`} />
          </div>
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-3 w-56 md:w-64 bg-white/90 backdrop-blur-2xl border border-white rounded-[20px] md:rounded-[24px] shadow-[0_10px_40px_rgba(0,0,0,0.08)] p-2 animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden">
              <div className="px-3 md:px-4 py-3 md:py-4 mb-2 border-b border-slate-100">
                <p className="text-[10px] md:text-[12px] font-black text-slate-400 uppercase tracking-widest mb-1">Mentor Account</p>
                <p className="text-[12px] md:text-[14px] font-bold text-[#0D3694] truncate">arjun.mehta@mentix.ai</p>
              </div>
